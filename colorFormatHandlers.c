...
void colorFormatHandlerL1BE(struct rgba8888_t* dst, uint8_t const* src, int numBytes) {
	for (int i = numBytes; i-- != 0;) {
		...
		for (int j = 0; j != 8; ++j) {
			...
			*dst++ = (struct rgba8888_t){a,a,a,0xff};
		}
	}
}
...
void colorFormatHandlerL2BE(struct rgba8888_t* dst, uint8_t const* src, int numBytes) {
	for (int i = numBytes; i-- != 0;) {
		...
		*dst++ = (struct rgba8888_t){a,a,a,0xff};
		*dst++ = (struct rgba8888_t){b,b,b,0xff};
		*dst++ = (struct rgba8888_t){c,c,c,0xff};
		*dst++ = (struct rgba8888_t){d,d,d,0xff};
	}
}
...
void colorFormatHandlerL4BE(struct rgba8888_t* dst, uint8_t const* src, int numBytes) {
	for (int i = numBytes; i-- != 0;) {
		...
		*dst++ = (struct rgba8888_t){hi,hi,hi,0xff};
		*dst++ = (struct rgba8888_t){lo,lo,lo,0xff};
	}
}
...
void colorFormatHandlerL8(struct rgba8888_t* dst, uint8_t const* src, int numBytes) {
	for (int i = numBytes; i-- != 0; ++dst, ++src) {
		*dst = (struct rgba8888_t){*src,*src,*src,0xff};
	}
}
...
void colorFormatHandlerRGB888(struct rgba8888_t* dst, uint8_t const* src, int numBytes) {
	while (numBytes >= 3) {
		...
		uint8_t const g = *src++;
		uint8_t const b = *src++;
		*dst++ = (struct rgba8888_t){r,g,b,0xff};
		numBytes -= 3;
	}
}
